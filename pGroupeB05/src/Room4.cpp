...
Room4::Room4(sf::RenderWindow& app,StateManager& stateManager,Player& p1): app(app),stateManager(stateManager),p1(p1)
{
    ...
    doorToR6 = sf::RectangleShape(sf::Vector2f(50, WALL_THICKNESS));
    doorToR6.setFillColor(sf::Color::Magenta);  // Vous pouvez choisir la couleur que vous préférez
    doorToR6.setPosition((WINDOW_WIDTH - 50) / 2, 0);  // Au milieu du mur supérieur
...
void Room4::update()
{
    for (int i=0;i<NUM_ENEMIES1_ROOM4;i++)
    {
        enemies[i].update(p1.getCircle().getPosition());
    }

    for (int i=0;i<NUM_ENEMIES2_ROOM4;i++)
    {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM4; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES1_ROOM4; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES2_ROOM4; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES2_ROOM4; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM4; i++)
    {
        for (int j = 0; j < NUM_ENEMIES2_ROOM4; j++)
        {
...
void Room4::render()
{
    ...
    for (int i = 0; i < NUM_OBSTACLES4; i++)
    {
...
void Room4::handleInput()
{
        ...
        if (p1.getCircle().getPosition().y <= doorToR6.getPosition().y + WALL_THICKNESS && p1.getCircle().getPosition().x >= door.getPosition().x
            && p1.getCircle().getPosition().x + 2 * p1.getCircle().getRadius() <= doorToR6.getPosition().x + 50) {
            stateManager.setState(stateManager.getStateRoom6());

            if(stateManager.getCurrentState()== stateManager.getStateRoom6())
            {
    ...
    for (int i = 0; i < MAX_PROJECTILES; ++i)
    {
        ...
        for (int i = 0; i < MAX_PROJECTILES; ++i)
        {
            for (int j = 0; j < NUM_ENEMIES1_ROOM4; ++j)
            {
        ...
        if(numDeadEnemies1==NUM_ENEMIES1_ROOM4 && numDeadEnemies2==NUM_ENEMIES2_ROOM4)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM4; i++)
    {
        for (int j = 0; j < NUM_OBSTACLES4; j++)
        {
...
bool Room4::areRectanglesColliding(const sf::RectangleShape& rect1, const sf::RectangleShape& rect2)
{
...
void Room4::handleEnemyCollision(Enemy& ene1, Enemy& ene2)
{
...
void Room4::handleEnemyCollision2(Enemy2& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
void Room4::handleEnemyCollision3(Enemy& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
bool Room4::isProjectileCollidingWithEnemy(const sf::CircleShape& projectile, const sf::RectangleShape& enemy)
{
...
bool Room4::isPlayerObstacle(const sf::CircleShape& player, const sf::RectangleShape& obstacle)
{
...
bool Room4::isCollidingWithObstacle(float x, float y, float radius) {
...
bool Room4::isPlayerCollidingWithEnemy(const sf::CircleShape& player, const sf::RectangleShape& enemy)
{
