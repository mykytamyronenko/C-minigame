...
Room6::Room6(sf::RenderWindow& app,StateManager& stateManager,Player& p1): app(app),stateManager(stateManager),p1(p1),iah(1750,100),tacos(600,950)
{
    ...
    doorToR7 = sf::RectangleShape(sf::Vector2f(30,60));
    doorToR7.setFillColor(sf::Color::Magenta);
    doorToR7.setPosition(0, (WINDOW_HEIGHT - WALL_THICKNESS) / 2);

    // Initialisation des obstacles
    obstacles[0] = sf::RectangleShape(sf::Vector2f(35, 70));
    obstacles[0].setFillColor(sf::Color::Transparent);
    obstacles[0].setPosition(0,(WINDOW_HEIGHT/2)-20);
    ...
    obstacles[2] = sf::RectangleShape(sf::Vector2f(1200, 400));
    obstacles[2].setPosition(400, 10);

    obstacles[3] = sf::RectangleShape(sf::Vector2f(200, 400));
    obstacles[3].setPosition(1600, 400);

    for(int i = 2; i<=NUM_OBSTACLES6; i++){
    ...
    enemies.push_back(Enemy(50.0f, 50.0f, enemyTexture));
    enemies.push_back(Enemy(50.0f, 100.0f, enemyTexture));
    enemies.push_back(Enemy(50.0f, 150.0f, enemyTexture));
    enemies.push_back(Enemy(50.0f, 200.0f, enemyTexture));
...
void Room6::update()
{
    for (int i=0;i<NUM_ENEMIES1_ROOM6;i++)
    {
        enemies[i].update(p1.getCircle().getPosition());
    }

    for (int i=0;i<NUM_ENEMIES2_ROOM6;i++)
    {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM6; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES1_ROOM6; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES2_ROOM6; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES2_ROOM6; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM6; i++)
    {
        for (int j = 0; j < NUM_ENEMIES2_ROOM6; j++)
        {
...
void Room6::render()
{
    ...
    for (int i = 0; i < NUM_OBSTACLES6; i++)
    {
...
void Room6::handleInput()
{
        ...
        if (p1.getCircle().getPosition().y + 2 * p1.getCircle().getRadius() >= doorToR7.getPosition().y &&
            p1.getCircle().getPosition().y <= doorToR7.getPosition().y + 50 &&
            p1.getCircle().getPosition().x <= doorToR7.getPosition().x + WALL_THICKNESS &&
            p1.getCircle().getPosition().x + 2 * p1.getCircle().getRadius() >= doorToR7.getPosition().x) {
                stateManager.setState(stateManager.getStateRoom7());
            if(stateManager.getCurrentState()== stateManager.getStateRoom7())
            {
    ...
    for (int i = 0; i < MAX_PROJECTILES; ++i)
    {
        ...
        for (int i = 0; i < MAX_PROJECTILES; ++i)
        {
            for (int j = 0; j < NUM_ENEMIES1_ROOM6; ++j)
            {
        ...
        if(numDeadEnemies1==NUM_ENEMIES1_ROOM6 && numDeadEnemies2==NUM_ENEMIES2_ROOM6)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM6; i++)
    {
        for (int j = 0; j < NUM_OBSTACLES6; j++)
        {
...
bool Room6::areRectanglesColliding(const sf::RectangleShape& rect1, const sf::RectangleShape& rect2)
{
...
void Room6::handleEnemyCollision(Enemy& ene1, Enemy& ene2)
{
...
void Room6::handleEnemyCollision2(Enemy2& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
void Room6::handleEnemyCollision3(Enemy& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
bool Room6::isProjectileCollidingWithEnemy(const sf::CircleShape& projectile, const sf::RectangleShape& enemy)
{
...
bool Room6::isPlayerObstacle(const sf::CircleShape& player, const sf::RectangleShape& obstacle)
{
...
bool Room6::isCollidingWithObstacle(float x, float y, float radius) {
...
bool Room6::isPlayerCollidingWithEnemy(const sf::CircleShape& player, const sf::RectangleShape& enemy)
{
