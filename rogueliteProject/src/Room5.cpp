...
Room5::Room5(sf::RenderWindow& app,StateManager& stateManager,Player& p1): app(app),stateManager(stateManager),p1(p1),iah(1700,800),tacos(1700,950)
{
    ...
    obstacles[0].setPosition(-20,(WINDOW_HEIGHT/2)-10);

    obstacles[1] = sf::RectangleShape(sf::Vector2f(1200, 400));
    obstacles[1].setFillColor(sf::Color::Black);
    obstacles[1].setPosition(400, 10);

    obstacles[2] = sf::RectangleShape(sf::Vector2f(1200, 400));
    obstacles[2].setFillColor(sf::Color::Black);
    obstacles[2].setPosition(400, 670);

    enemyTexture.loadFromFile("Images/BaseSlime.png");
    enemies.push_back(Enemy(1600.0f, 550.0f, enemyTexture));
    enemies.push_back(Enemy(1500.0f, 500.0f, enemyTexture));

    enemyTexture2.loadFromFile("Images/skeleton.png");
    enemies2.push_back(Enemy2(1800.0f, 10.0f, enemyTexture2));
...
void Room5::update()
{
    for (int i=0;i<NUM_ENEMIES1_ROOM5;i++)
    {
        enemies[i].update(p1.getCircle().getPosition());
    }

    for (int i=0;i<NUM_ENEMIES2_ROOM5;i++)
    {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM5; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES1_ROOM5; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES2_ROOM5; i++)
    {
        for (int j = i + 1; j < NUM_ENEMIES2_ROOM5; j++)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM5; i++)
    {
        for (int j = 0; j < NUM_ENEMIES2_ROOM5; j++)
        {
...
void Room5::render()
{
    ...
    for (int i = 0; i < NUM_OBSTACLES5; i++)
    {
...
void Room5::handleInput()
{
    ...
    for (int i = 0; i < MAX_PROJECTILES; ++i)
    {
        ...
        for (int i = 0; i < MAX_PROJECTILES; ++i)
        {
            for (int j = 0; j < NUM_ENEMIES1_ROOM5; ++j)
            {
        ...
        if(numDeadEnemies1==NUM_ENEMIES1_ROOM5 && numDeadEnemies2==NUM_ENEMIES2_ROOM5)
        {
    ...
    for (int i = 0; i < NUM_ENEMIES1_ROOM5; i++)
    {
        for (int j = 0; j < NUM_OBSTACLES5; j++)
        {
...
bool Room5::areRectanglesColliding(const sf::RectangleShape& rect1, const sf::RectangleShape& rect2)
{
...
void Room5::handleEnemyCollision(Enemy& ene1, Enemy& ene2)
{
...
void Room5::handleEnemyCollision2(Enemy2& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
void Room5::handleEnemyCollision3(Enemy& ene1, Enemy2& ene2)
{
    if (areRectanglesColliding(ene1.getEne(), ene2.getEne()))
    {
        // Déplacement des ennemis dans des directions opposées
...
bool Room5::isProjectileCollidingWithEnemy(const sf::CircleShape& projectile, const sf::RectangleShape& enemy)
{
...
bool Room5::isPlayerObstacle(const sf::CircleShape& player, const sf::RectangleShape& obstacle)
{
...
bool Room5::isCollidingWithObstacle(float x, float y, float radius) {
...
bool Room5::isPlayerCollidingWithEnemy(const sf::CircleShape& player, const sf::RectangleShape& enemy)
{
